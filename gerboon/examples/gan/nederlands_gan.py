...
# Build Generator
gen_opt = ks.optimizers.Adam(0.001)
gen_model = ks.models.Sequential()
gen_model.add(ks.layers.LSTM(64, input_shape=(word_length, 5), return_sequences=True))
gen_model.add(ks.layers.LSTM(64, return_sequences=True))
gen_model.add(ks.layers.Dense(len(alphabet), activation=ks.activations.softmax))
...
discr_model.add(ks.layers.LSTM(64, return_sequences=True))
discr_model.add(ks.layers.LSTM(64))
discr_model.add(ks.layers.Dense(2, activation=ks.activations.softmax))
discr_model.compile(optimizer=ks.optimizers.Adam(0.002), loss=ks.losses.categorical_crossentropy)
...
for i in range(1000):
    ...
    epoch = np.concatenate([words_one_hot[:epoch_size // 2], words], axis=0)
    y = np.zeros([epoch_size, 2])
    y[:epoch_size // 2, 0] = 1
    y[epoch_size // 2:, 1] = 1
    print("Training discriminative model")
    discr_model.fit(epoch, y, epochs=1, batch_size=200)
