# From https://weiminwang.blog/2017/09/29/multivariate-time-series-forecast-using-seq2seq-in-tensorflow/#multivariate
...
# Get 105 random points from 0 to 30
x = np.linspace(0, 30, 105)
# The training data will be all the random points minus the length of the output sequence
train_data_x = x[:(105-output_seq_len)]


# Helper function to get the "true" signal
def true_signal(x):
    y = 2 * np.sin(x)
    return y


# Helper function to get noise for the size of our array.
def noise_func(x, noise_factor = 0.5):
    return np.random.randn(len(x)) * noise_factor


# Generate the signals used for training
...
def generate_train_samples(x=train_data_x, batch_size=10, input_seq_len=15, output_seq_len=20):
    ...
    # Get batch_size amount of random points in the range of our starting points.
    start_x_idx = np.random.choice(range(total_start_points), batch_size)

    # Make batch_size amount of random input sequences from start point for the length of the input sequence
    input_seq_x = [x[i:(i + input_seq_len)] for i in start_x_idx]
    # Make the corresponding output sequence
...
with tf.Session() as sess:
    ...
    save_path = temp_saver.save(sess, os.path.join('/home/mauk/Workspace/KerasMetMoek/moek/examples', 'univariate_ts_model0'))
...
with tf.Session() as sess:
    ...
    saver = rnn_model['saver']().restore(sess, os.path.join('/home/mauk/Workspace/KerasMetMoek/moek/examples', 'univariate_ts_model0'))
...
plt.legend(handles = [l1, l3], loc = 'lower left')
plt.show()